...
const ARTICLE = {
  ...
  content: `
    <p style="font-size:19px;font-weight:600;color:#0d2d52;line-height:1.8;margin-bottom:28px;border-left:4px solid #1c4f8a;padding-left:20px;">봄비가 살짝 내린 4월 셋째 주 토요일, 경기도 이천 설봉공원 일대는 도자기와 꽃의 향연으로 가득 찼다. 올해로 열 번째를 맞은 이천도자기축제는 단순한 관광 행사를 넘어, 한국 전통 도예의 숨결이 살아 숨쉬는 문화 현장으로 자리매김했다.</p>
...
const VIDEOS = [
  { id:"v1", title:"이천도자기축제 현장 스케치 — 장인의 손", duration:"3:24", thumb:"https://images.unsplash.com/photo-1565193566173-7a0ee3dbe261?w=400&q=80" },
  { id:"v2", title:"닥터리부트 원장이 전하는 두피 건강 비법", duration:"5:12", thumb:"https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?w=400&q=80" },
...
const INIT_COMMENTS = [
  { id:1, name:"김미선", date:"2026.04.28", content:"정말 생생한 취재네요! 이천 도자기축제 꼭 가보고 싶어졌어요 😊", likes:8 },
  { id:2, name:"박철수", date:"2026.04.28", content:"장인 김광수 선생님 시연 영상도 있으면 좋겠어요!", likes:5 },
  { id:3, name:"이순희", date:"2026.04.29", content:"봉숭아학당 시민기자분 이야기가 인상적이었어요.", likes:12 },
];

const RELATED = [
  { id:"2", title:"27년째 골목을 지키는 손 — 닥터리부트 라온 원장", channel:"이음피플", views:892, thumb:"https://images.unsplash.com/photo-1519085360753-af0119f7cbe7?w=120&q=80" },
  { id:"3", title:"바이올리니스트 박진영, 금호아트홀 독주회", channel:"이음피플", views:516, thumb:"https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=120&q=80" },
  { id:"4", title:"우울증을 이겨낸 13만 유튜버, 태리TV 윤진희", channel:"이음피플", views:724, thumb:"https://images.unsplash.com/photo-1531746020798-e6953c6e8e04?w=120&q=80" },
];
...
const POPULAR = [
  ...
  { id:"p2", title:"넷플릭스의 '문화 식민지'로 살 것인가", channel:"이음매거진", views:886, thumb:"https://images.unsplash.com/photo-1536440136628-849c177e76a1?w=80&q=80" },
  ...
  { id:"p4", title:"AI 시대, 우리 아이 교육은 어디로", channel:"이음뷰", views:1102, thumb:"https://images.unsplash.com/photo-1503676260728-1c00da094a0b?w=80&q=80" },
  { id:"p5", title:"바이올리니스트 박진영, 금호아트홀", channel:"이음피플", views:516, thumb:"https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?w=80&q=80" },
];

function FloatBtn({ onClick, children, active, title }) {
  const [hover, setHover] = useState(false);
  return (
    <button onClick={onClick} title={title}
      onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      style={{width:"40px",height:"40px",borderRadius:"50%",background:active?"#1c4f8a":hover?"#0d2d52":"#fff",border:`1px solid ${active||hover?"#0d2d52":"#ddd"}`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:hover?"0 4px 12px rgba(13,45,82,0.25)":"0 2px 6px rgba(0,0,0,0.1)",transition:"all 0.2s",gap:"1px",color:active||hover?"white":"#555"}}>
      {children}
    </button>
  );
}
...
function NavBtn({ onClick, dir }) {
  ...
  return (
    <button onClick={onClick}
      onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}
      style={{width:"34px",height:"34px",borderRadius:"50%",background:hover?"#0d2d52":"#fff",border:`2px solid ${hover?"#0d2d52":"#ddd"}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",fontSize:"18px",color:hover?"white":"#0d2d52",transition:"all 0.2s",fontWeight:"900",lineHeight:1}}>
      {dir==="prev" ? "‹" : "›"}
    </button>
  );
}
...
export default function ArticleDetail() {
  ...
  const prevVideo = () => setVideoIdx(p=>(p-1+VIDEOS.length)%VIDEOS.length);
  const nextVideo = () => setVideoIdx(p=>(p+1)%VIDEOS.length);
  const prevCard  = () => setCardIdx(p=>(p-1+CARDS.length)%CARDS.length);
  const nextCard  = () => setCardIdx(p=>(p+1)%CARDS.length);

  return (
    <div style={{fontFamily:"'Noto Sans KR',sans-serif",background:"#fff",color:"#1a1a1a",minHeight:"100vh"}}>

      {/* 플로팅 바 */}
      <div style={{position:"fixed",right:"16px",top:"50%",transform:"translateY(-50%)",display:"flex",flexDirection:"column",alignItems:"center",gap:"6px",zIndex:999}}>
        <FloatBtn onClick={onLike} active={liked} title="좋아요">
          <span style={{fontSize:"14px"}}>{liked?"❤️":"🤍"}</span>
          <span style={{fontSize:"8px",fontWeight:"700"}}>{likeCount}</span>
        </FloatBtn>
        <FloatBtn title="댓글">
          <span style={{fontSize:"14px"}}>💬</span>
          <span style={{fontSize:"8px",fontWeight:"700"}}>{comments.length}</span>
        </FloatBtn>
        <FloatBtn onClick={onBookmark} active={bookmarked} title="찜하기">
          <span style={{fontSize:"14px"}}>{bookmarked?"🔖":"📌"}</span>
        </FloatBtn>
        <div style={{width:"1px",height:"12px",background:"#ddd"}}/>
        <FloatBtn onClick={onKakao} title="카카오 공유">
          <span style={{fontSize:"12px",fontWeight:"700",color:"#3C1E1E"}}>K</span>
        </FloatBtn>
        <FloatBtn onClick={onFb} title="페이스북">
          <span style={{fontSize:"12px",fontWeight:"700"}}>f</span>
        </FloatBtn>
        <FloatBtn onClick={onCopy} active={copied} title="링크복사">
      ...
      <div style={{background:"#0d2d52",padding:"6px 0",fontSize:"11px"}}>
        <div style={{maxWidth:"1200px",margin:"0 auto",padding:"0 32px",display:"flex",justifyContent:"space-between",color:"rgba(255,255,255,0.6)"}}>
          ...
          <div style={{display:"flex",gap:"16px"}}>
            {["시민기자 지원","광고문의","로그인"].map(t=>(<a key={t} href="#" style={{color:"rgba(255,255,255,0.55)",textDecoration:"none"}}>{t}</a>))}
          </div>
        </div>
      </div>
      ...
      <header style={{background:"#fff",borderBottom:"1px solid #e0e0e0"}}>
        <div style={{maxWidth:"1200px",margin:"0 auto",padding:"0 32px"}}>
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"20px 0 14px",borderBottom:"3px solid #0d2d52"}}>
            <Link to="/" style={{textDecoration:"none"}}>
              <div style={{fontFamily:"serif",fontSize:"30px",fontWeight:"700",color:"#0d2d52",letterSpacing:"-1px"}}>이<span style={{color:"#e8432d",textShadow:"0 0 12px rgba(232,67,45,0.4)"}}>음</span>미디어</div>
              <div style={{fontSize:"10px",color:"#9a9a9a",letterSpacing:"3px"}}>E-EUM MEDIA</div>
            </Link>
          ...
          <nav>
            <ul style={{display:"flex",listStyle:"none",margin:0,padding:0}}>
              {["전체","이음매거진","이음로컬","이음에듀","이음피플","이음트렌드","이음보이스","이음뷰"].map(ch=>(
                <li key={ch}><Link to={ch==="전체"?"/":"/"+ch} style={{display:"block",padding:"11px 16px",fontSize:"12.5px",fontWeight:ch===a.channel?"700":"500",color:ch===a.channel?"#1c4f8a":"#3a3a3a",textDecoration:"none",borderBottom:ch===a.channel?"2px solid #1c4f8a":"2px solid transparent"}}>{ch}</Link></li>
              ))}
            </ul>
          </nav>
        </div>
      ...
      <div style={{background:"#f7f8fa",borderBottom:"1px solid #e8e8e8",padding:"10px 0",fontSize:"11px",color:"#9a9a9a"}}>
        <div style={{maxWidth:"1200px",margin:"0 auto",padding:"0 32px",display:"flex",gap:"6px",alignItems:"center"}}>
          <Link to="/" style={{color:"#1c4f8a",textDecoration:"none"}}>홈</Link><span>›</span>
          <Link to={"/"+a.channel} style={{color:"#1c4f8a",textDecoration:"none"}}>{a.channel}</Link><span>›</span>
          <span>{a.title}</span>
        </div>
      </div>
      ...
      <div style={{maxWidth:"1200px",margin:"0 auto",padding:"48px 32px",display:"grid",gridTemplateColumns:"1fr 300px",gap:"56px",alignItems:"start"}}>
        <main>
          ...
          <div style={{border:"1px solid #e0e0e0",padding:"24px",margin:"32px 0",background:"#f7f8fa"}}>
            ...
            <div style={{display:"flex",gap:"16px",alignItems:"flex-start"}}>
              ...
              <div>
                ...
                <div style={{fontSize:"13px",color:"#3a3a3a",lineHeight:"1.7"}}>{a.author_intro}</div>
              </div>
            </div>
          </div>
          ...
          <div style={{margin:"32px 0"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px"}}>
              ...
              {/* 영상 */}
              <div>
                <div style={{position:"relative",height:"180px",borderRadius:"8px",overflow:"hidden",marginBottom:"10px"}}>
                  ...
                  <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.35)",display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <div style={{width:"48px",height:"48px",borderRadius:"50%",background:"rgba(255,255,255,0.9)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"18px",cursor:"pointer"}}>▶</div>
                  </div>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"12px"}}>
                  <NavBtn onClick={prevVideo} dir="prev"/>
                  <span style={{fontSize:"11px",color:"#9a9a9a"}}>{videoIdx+1} / {VIDEOS.length}</span>
                  <NavBtn onClick={nextVideo} dir="next"/>
                </div>
              </div>

              {/* 카드뉴스 */}
              <div>
                <div style={{position:"relative",height:"180px",borderRadius:"8px",overflow:"hidden",marginBottom:"10px",cursor:"pointer"}}>
                  <img src={CARDS[cardIdx].thumb} alt={CARDS[cardIdx].title} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                  <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.3)"}}/>
                  <span style={{position:"absolute",top:"8px",left:"8px",background:"rgba(255,255,255,0.2)",color:"white",fontSize:"9px",fontWeight:"700",padding:"3px 8px",borderRadius:"3px"}}>🃏 카드뉴스</span>
        ...
        <aside style={{display:"flex",flexDirection:"column",gap:"24px",position:"sticky",top:"20px"}}>
          ...
          {/* 구독 박스 */}
          <div style={{background:"linear-gradient(135deg,#0d2d52,#1c4f8a)",borderRadius:"12px",padding:"24px",textAlign:"center",boxShadow:"0 4px 16px rgba(13,45,82,0.3)"}}>
            <img src="/logo.png" alt="이음미디어" style={{height:"40px",marginBottom:"12px"}}/>
            <div style={{color:"white",fontWeight:"700",fontSize:"15px",marginBottom:"6px"}}>이음미디어 구독</div>
            <div style={{color:"rgba(255,255,255,0.7)",fontSize:"12px",lineHeight:"1.7",marginBottom:"16px"}}>새 기사 발행 시<br/>카카오톡으로 바로 알림!</div>
            <button onClick={onKakao} style={{width:"100%",background:"#FEE500",color:"#3C1E1E",border:"none",padding:"12px",fontSize:"13px",fontWeight:"700",cursor:"pointer",fontFamily:"inherit",borderRadius:"6px",display:"flex",alignItems:"center",justifyContent:"center",gap:"6px"}}>
              💬 카카오 채널 구독 (무료)
            </button>
          </div>
          ...
          {/* 닥터리부트 광고 */}
          <div style={{background:"#f7f8fa",border:"1px solid #e0e0e0",padding:"16px",borderRadius:"8px"}}>
            <div style={{fontSize:"9px",color:"#9a9a9a",letterSpacing:"1px",marginBottom:"10px"}}>AD</div>
            <div style={{width:"100%",height:"120px",background:"linear-gradient(135deg,#0d2d52,#1c4f8a)",display:"flex",alignItems:"center",justifyContent:"center",marginBottom:"12px",borderRadius:"6px"}}>
            ...
            <button style={{width:"100%",background:"#0d2d52",color:"white",border:"none",padding:"9px",fontSize:"11px",fontWeight:"700",cursor:"pointer",fontFamily:"inherit",borderRadius:"4px"}}>예약 · 문의 →</button>
          </div>
