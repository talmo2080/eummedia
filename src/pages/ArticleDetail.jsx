...
// 캡션에서 출처 표기를 제거해 alt 텍스트로 재사용 (하위 호환용 fallback)
//   대응 형식 (문자열 끝):
//     · 사진 = ...
//     · 사진:...
//     · 〈사진 = ○○ 제공〉  · 〈사진=○○〉
//     · (사진=○○) / [사진=○○]
//   예외: 〈AI 생성 편집 이미지〉 / (AI 생성) → 보존 (AI 표기는 alt에도 남기는 것이 정합)
export function cleanCaptionForAlt(caption) {
  if (!caption) return '';
  const originals = [];
  // AI 생성 표기는 sentinel로 임시 치환 → 출처 정규식이 삼키지 못하게
  let s = String(caption).replace(/[〈(]\s*AI\s*생성[^〉)\]]*[〉)\]]/g, (m) => {
    originals.push(m);
    return `AI${originals.length - 1}`;
  });
  // 문자열 끝의 출처 표기 제거
  //   [ ]?  ← 앞 여닫이 부호 (선택)
  //   사진  ← 리터럴
...
function paragraphToHtml(p) {
  ...
  //     · 없으면 캡션에서 출처 표기 제거한 값 사용 (하위 호환)
  //         · "사진=...", "사진 = ...", "사진:..."
  //         · 꺾쇠·괄호: 〈사진=...〉 · (사진=...) · [사진=...]
  //         · 예외: 〈AI 생성 편집 이미지〉는 보존 (AI 표기는 alt에도 남기는 것이 정합)
  //     · 둘 다 비면 alt="" (빈 문자열, 스크린리더가 skip)
  //         · '본문 이미지' 같은 무의미 기본값은 접근성상 빈 alt보다 나쁨
  m = text.match(/^\[이미지:([^\]]+)\]$/);
  if (m) {
    // split 방식 파싱 (정규식 슬롯 분리 대신):
    //   · 1조각 → URL만
    //   · 2조각 → URL|캡션
    //   · 3조각 이상 → URL|캡션...|alt (마지막이 alt, 중간 파이프는 캡션에 join)
    //   이렇게 하면 캡션 안에 '|' 문자가 있어도 파싱 깨지지 않음
    //   (예: "대둔산 케이블카 | 사진=이음미디어" 같은 캡션 지원)
    ...
    if (parts.length === 2) {
      ...
      altExplicit = parts[parts.length - 1].trim();
    ...
    if (/^https:\/\//.test(url)) {
      ...
      const captionForAlt = cleanCaptionForAlt(caption);
    ...
    <div style={{ fontFamily:"'Noto Sans KR',sans-serif", background:"#fff", color:"#1a1a1a", minHeight:"100vh" }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[60px_1fr_300px]" style={{ maxWidth:"1280px", margin:"0 auto", padding:"48px 32px", gap:"32px", alignItems:"start" }}>
        ...
        <main id="main-content">
          ...
          {/* ━━━━━━━━ 채널 배지 (반응형 · 두 스타일 중 하나만 시각 노출) ━━━━━━━━ */}
          {/* 데스크탑: 얇은 라인 + 채널명 (매체별 컬러) */}
          <div className="hidden lg:flex" style={{ fontSize:"10px", color:color, fontWeight:"700", letterSpacing:"2px", textTransform:"uppercase", marginBottom:"14px", alignItems:"center", gap:"8px" }} aria-hidden="true">
            <span style={{ width:"24px", height:"1px", background:color, display:"inline-block" }} />{a.channel}
          </div>
          {/* 모바일: 컬러 배지 */}
          <span className={`lg:hidden inline-block ${getChannelColorClasses(a.channel)} text-[11px] font-bold px-2 py-1 rounded tracking-wider mb-3`} aria-hidden="true">
            {a.channel}
          </span>

          {/* ━━━━━━━━ 기사 제목 (h1 하나만, 반응형 스타일) ━━━━━━━━ */}
          {/* 시각 스타일 데스크탑(34px, 명조 NAVY) / 모바일(24px, 명조 검정) — 실제 태그는 <h1> 하나 */}
          <h1
            className="font-serif font-bold tracking-tight text-[24px] leading-[1.3] text-neutral-900 mb-3 lg:text-[34px] lg:leading-[1.4] lg:mb-4"
            style={{ letterSpacing: "-1px", color: undefined }}
          >
            <span className="lg:hidden">{a.title}</span>
            <span className="hidden lg:inline" style={{ color:"#0d2d52" }}>{a.title}</span>
          </h1>

          {/* ━━━━━━━━ 부제 (반응형 스타일) ━━━━━━━━ */}
          <div className="font-serif italic border-l-[3px] border-blue-700 pl-4 mb-4 text-[15px] leading-[1.6] text-neutral-600 lg:text-[17px] lg:mb-6" style={{ color: undefined }}>
            <span className="lg:hidden">{a.subtitle}</span>
            <span className="hidden lg:inline" style={{ color:"#5a5a5a" }}>{a.subtitle}</span>
          </div>

          {/* ━━━━━━━━ 기자 메타 (데스크탑 카드형) ━━━━━━━━ */}
          <div className="hidden lg:flex" style={{ alignItems:"center", gap:"14px", padding:"16px 0", borderTop:"1px solid #e0e0e0", borderBottom:"1px solid #e0e0e0", marginBottom:"28px" }}>
            ...
            <div style={{ marginLeft:"auto", textAlign:"right", fontSize:"11px", color:"#595959", lineHeight:"1.8" }}>
              ...
              <div>
                ...
                <span className="sr-only">읽기 시간 </span>
                {a.read_time}분
                {a.view_count >= 100 && (
                  <>
                    {' · '}
          ...
          {/* ━━━━━━━━ 기자 메타 (모바일 한줄) ━━━━━━━━ */}
          <div className="lg:hidden my-3 text-sm text-neutral-600 leading-relaxed">
            <span className="font-bold text-neutral-900">{a.author_name}</span>
            <span className="mx-1.5 text-neutral-400" aria-hidden="true">·</span>
            ...
            {a.view_count >= 100 && (
              <>
                <span className="mx-1.5 text-neutral-400" aria-hidden="true">·</span>
                <span><span aria-hidden="true">👁 </span><span className="sr-only">조회수 </span>{a.view_count.toLocaleString()}</span>
              </>
            )}
          </div>
