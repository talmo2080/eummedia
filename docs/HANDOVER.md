...
## 📣 광고 마케팅 툴 시스템 (미진행, 5/15 박제)

> 5/15 세연님이 매체 자율성 + 수익 직결성 점검 중 발견. 정식 의논 0건 — 다음 사이클(1주차) 시작 영역.

### 5가지 광고 도구

| # | 도구 | 위치 | 비고 |
|---|---|---|---|
| 1 | 광고기사 | 채널/본문 | `articles.is_sponsored` 컬럼 + SPONSORED 배지 (P-03 별도 사이클로 박제됨) |
| 2 | 중간배너 | 기사 본문 중간 | 2주차 1차 구현 후보 |
| 3 | 카드뉴스 | 기사 하단 | 클릭 시 해당 기사로 이동 (네이티브 광고 패턴) |
| 4 | 동영상 | 풀패키지 | Phase B 영역 (창간 후) |
| 5 | 사이드배너 | 우측 사이드 | ArticleDetail/Home 사이드 영역 |
...
- `advertisements` 테이블 — 기존 마이그레이션에 SQL 준비됨 (적용 여부 미확인, HANDOVER 알려진 이슈 #4)
- `articles.is_sponsored` boolean 컬럼 — P-03 SPONSORED 배지와 통합 후보

### ⏭ 다음 사이클 시작 영역

1주차 첫 영역 = 광고 시스템 큰 설계 의논. 의논 항목:
1. 도구별 우선순위 (창간 시점 어디부터?)
2. 단가 확정 (협상 여지 + 운영비 기반)
3. DB 구조 (advertisements 테이블 vs is_sponsored 단일 컬럼 vs 혼합)
