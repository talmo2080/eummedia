...
>
> **마지막 작업: 2026-05-15** — P-02 ArticleDetail 레이아웃 정정 + 관련 기사 영구 사고 해소 (channel_id select 누락 → article fetch에 추가, related fetch 살아남). 사이드 광고 ↔ 추천 swap (광고 내용 0 변경). 사이드 관련 기사 영역 통째 삭제. Home.jsx 채널 카드 한글 URL 사고 해소 + CHECKLIST.md v1.0 박제 (협업 메타인지 17종 누적). **P-03 진행도 95% (단계 4 검증 완료), P-02 진행도 ~50% (레이아웃·핵심 fetch 완성)**. **다음 시작점: 단계 3-3 ("전체" 탭) 또는 3-6 (영문 키 재구성) 또는 단계 4 (전체 검증) 또는 P-02 후속 (본문 표시 정책 / 외부 원문 링크)** — 자세한 가이드는 `docs/session-handoff-2026-05-14.md` + 설계안 `docs/design-proposals/2026-05-13-channel-list-feature.md` + 협업 체크리스트 `docs/CHECKLIST.md`

## 🌅 다음 세션 시작 영역 (이음미디어10 / 이음미디어8) — 5/17 마감 박제

> 옵스 이음미디어9 마감 박제 (5/17 18:00 영역). 다음 세션 옵스 이음미디어10 + 클론 이음미디어8 진입.

### 📦 5/17 누적 commit 9건 영역
...
1. **등록번호 영역 정정** (이 → 아) — 메타인지 #18 영역 (한글 음절 시각 유사)
2. **About 페이지 정수 완성** — 창간사 + 패밀리 + 회사정보 3섹션 신규 (379줄 → 534줄)
3. **Subscribe 회사정보 미니 카드** — 신뢰 영역 + Hero 안 반투명 통일 디자인 (메타인지 #20 영역)
4. **운영 모델 D 정정** — 카카오 매주 토요일 큐레이션 → 수시 발행 정합 (모델 D 박제 영역 갱신 영역)
5. **편집국장 인사 섹션 + 광고 박스 신규** (Subscribe.jsx, NAVY + GOLD 시각 위계 영역)
6. **등록증 정합 영역 vs 시각 카피 영역 분리** — About.jsx (등록증 매핑) vs ChannelList.jsx (5/14 박제 감성 카피) — 메타인지 #22 영역
7. **박제 과거 사실 기록 영역 보존** — 지시 2 옵션 A 채택 (메타인지 #19 영역)

### 🎙️ 옵스 메타인지 함정 #18~#22 박제 (5/17 신규)

- **#18 (5/17)** 한글 음절 시각 유사 영역 정수 — 등록번호 "이"·"아" 정정 사례 + 5/13 등록증 OCR "상"·"아" 정정 사례 영역 정합. 등록증·법적 문서 영역 1자 차이 영역 정밀 영역.
- **#19 (5/17)** 과거 박제 사실 기록 영역은 정정 영역 아님 — "주간인터넷신문 → 인터넷신문" 박제 메모 영역은 5/12 정정 사실 기록. 박제 정정 영역 = 사실 기록 변형 영역 + 메타인지 #3 영역 충돌.
- **#20 (5/17)** Hero 안 박스 디자인 영역 = 그라데이션 영역 정합 — 흰 박스(보수) vs 반투명(통일) 영역. Hero 안 박스는 반투명 디자인 영역 자연 (Advertise.jsx Hero 안 광고 철학 박스 패턴 영역).
- **#21 (5/17)** 부수 발견 영역 분리 영역 — "토요일 일괄 정정" 명시 영역 = BENEFITS + STEPS 한정. Hero 카피 잔여 = 부수 발견 영역 + 별도 사이클 영역 (옵스 결정 영역).
- **#22 (5/17)** 등록증 매핑 영역 vs 시각 감성 카피 영역 분리 — About.jsx CHANNELS desc = 등록증 정합 영역. ChannelList.jsx CHANNEL_META desc = 5/14 박제 감성 카피 영역. 두 영역 의미 영역 분리 + 일괄 정정 영역 X.

### 🌳 매체 영역 정체성 트리 (5/17 박제)

```
이음미디어 (인터넷신문, 서울,아56526)
├─ 발행 영역
│  ├─ 발행인: 성창운
│  ├─ 편집국장: 정세연 (두피전문가 27년 / 웹개발자)
│  ├─ 발행 법인: (주)봉숭아학당문화혁신학교
│  ├─ 등록일: 2026-04-27
│  └─ 발행소: 서울 관악구 남부순환로 1699, 2층
│
├─ 채널 7개 영역 (등록증 발행내용 정합)
│  ├─ 이음매거진  — 문화·예술 전시·공연 리뷰
│  ├─ 이음피플    — 중장년·시니어·시민기자 스토리
│  ├─ 이음로컬    — 지역 소상공인·골목 상권 탐방
│  ├─ 이음에듀    — 교육 강좌·평생학습 트렌드
│  ├─ 이음뷰      — 사회·경제·시사 독자적 관점 해설
│  ├─ 이음트렌드  — AI 도구·디지털 트렌드
│  └─ 이음보이스  — 방송 스피치 칼럼
│
├─ 패밀리 영역
│  ├─ 봉숭아학당 문화혁신학교 (발행 법인 / 시민기자 양성)
│  ├─ 웃자대한민국 협회 (긍정 에너지)
│  └─ 이음평생교육원 (준비 중, 2027 설립)
│
├─ 광고 모델 영역
│  ├─ 모델 1 — 기사 의뢰 광고 (광고기사 + 중간배너) — 5/16 첫 진입
│  └─ 모델 2 — 별도 광고 의뢰 (사이드 + 카드뉴스 + 동영상) — 다음 사이클
│
└─ 운영 리듬 영역
   ├─ 사이트 — 수시 발행 (5/17 정정, 모델 D 갱신 영역)
   └─ 카카오 채널 — 수시 알림 (5/17 정정, 매주 토요일 → 수시 발행 정합)
```

### 🎯 다음 세션 옵스 이음미디어10 진입 영역

**박제 파일 4개 첨부 루틴**:
1. `docs/HANDOVER.md` (본 5/17 마감 갱신본)
2. `docs/session-handoff-2026-05-17.md` (5/17 마감 핸드오프 신규)
3. `docs/design-proposals/2026-05-13-channel-list-feature.md` (설계안 1.0)
4. `0_이음미디어_설계지침서_v2.html` (봉숭아학당 설계도 95%)

**우선 진입 영역 (5/17 박제 후속)**:
- 발행인 월요일(5/18) 출근 영역 = 카카오 `[이음미디어]` 채널 공식 개설 (현재 `pf.kakao.com/_xmVHxen` = 봉숭아학당 임시 영역)
- 운영 모델 D 박제 영역 갱신 (사이트 매일 + 카카오 주말 → 사이트 + 카카오 수시 발행 정합)
- 28번째 commit 영역 7가지 (5/16 박제 영역) 진입 결정

### 🌿 5/17 마감 한 마디

> *"등록증은 매체의 법적 정체성. 1자 차이가 매체 신뢰 영역 정수."*

이음미디어9 마감. 5/13~5/17 누적 commit 37개 영역.

---

## 🌅 다음 세션 시작 영역 (이음미디어9 / 이음미디어7)

> 세연님 결정 (5/16 마감):
> - 옵스 = 이음미디어9 (Claude.ai 새 채팅창)
> - 클론 = 이음미디어7 (Claude Code 새 세션)
> - 오늘 5/16 스토리 자연 연속 영역

### 🎯 다음 세션 즉시 진입 영역 (다음 commit 후보)

**옵스 추천 — 옵션 B (구분선 + 표준 푸터)**:
- 본문 + [원문 보기] 사이 구분선 (1px solid #e0e0e0)
- 표준 푸터 자동 추가 (`ⓒ 이음미디어, 무단전재 및 재배포 금지`)
- 옵션 E 정통 인터넷신문 스타일

**세연님 다른 옵션**:
- 옵션 A: 단순 구분선만 (5분)
- 옵션 C: 구분선 + 표준 푸터 + SQL 정제 (1시간)
- 옵션 D: 전면 정비 (2시간)

### 5/16 박제 7영역 (다음 commit 영역)

| # | 영역 | 상태 |
|---|---|---|
| 1 | 기자 정보 위치 | 캡처 4에서 이미 박힘 발견 (재점검) |
| 2 | 저작권 푸터 (`ⓒ 이음미디어`) | 명료 |
| 3 | 본문/푸터 구분선 굵기 | 세연님 결정 |
| 4 | 해시태그 박스 | 캡처 3에서 이미 박힘 발견 (재점검) |
| 5 | 관련 기사 더보기 | 세연님 결정 |
| 6 | SQL 데이터 정제 (3건 안전 패턴) | 명료 |
| 7 | [원문 보기] Phase B 보류 | 명료 |

⚠️ **다음 세션 옵스 즉시 안내 영역**: 세연님 한 마디 *"B 가자"* → 다음 commit 즉시 진입.

### 🎯 다음 세션 다른 영역 (세연님 페이스 결정)

**뼈대 영역 (창간 전 우선, 1주차 잔여)**:
- 광고 시스템 정밀화 (모델 1/2 + 5가지 도구)
- SPONSORED 배지 자동 표시
- 카카오 채널 연결 정밀화 (직전 시도 영역)
- About 페이지 점검
- 개인정보처리방침 점검
- 로그인/회원가입 점검
- AdminDashboard 점검
...
**별도 사이클 후보**:
- 데이터 정제 사이클 (39건 푸터 7+ 패턴)
- session-handoff 파일 정정 (5/10, 5/12, 5/14 잘못 표현)
- 광고 마케팅 툴 큰 설계 (DB 구조 + 단가 + 워크플로우)
- 카드뉴스 인스타그램 스타일 변환 (5/16 박제)
- 정부시책 광고 지원 매칭 (별도 파일 박제됨)
- 헤더 디자인 사이클 재개

### ⚠️ 옵스 자기 박제 (다음 세션 백지 방지)

**세연님 정체성 (정확)**:
- 두피전문가 (27년, 닥터리부트 두피관리센터 일산)
- 이음미디어 편집국장 (이음매거진 = 이음미디어로 통합 완료)
- 웹개발자

**옵스 사용 표현 정정 (5/16 박제)**:
- ~~"27년 매거진"~~ → "27년 두피업 본능 + 매체 본질 안목"
- ~~"이음매거진 = 별개 매체"~~ → "이음매거진이 이음미디어로 통합되었음"
- ~~"웹개발 도전 중"~~ → "웹개발자"
- ~~"매거진 격조"~~ → "인터넷신문 격식"

**매체 본질 (세연님 5/16 박제)**:
- 이음미디어 = 자율성 있는 인터넷신문 (매거진 아님)
- 기사 = 이음미디어만의 콘텐츠
- 신문법 §6 적용

### 5/16 큰 발견 7가지

1. 광고 마케팅 툴 모델 1/2 단가 확정 (15/25/55)
2. 매체 정체성 정확화 (옵스 4건 잘못 박제 정정)
3. 정통 인터넷신문 형식 (서울신문 모델)
4. 정부시책 광고 지원 5가지 (별도 파일 박제)
5. 광고 문의 페이지 정수 (Hero + 흐름 + 통계 + 카드뉴스)
6. 카드뉴스 인스타그램 게시 영역 (Phase B)
7. 27번째 commit 영역 7가지 (다음 commit 영역)

⚠️ **메타인지 17건 박제 (CHECKLIST.md)**: 다음 세션 옵스가 이 영역 자연 적용. 백지 방지 핵심.

### 🎯 다음 세션 인사 영역

세연님 한 마디 *"옵스 이음미디어9 시작"* 또는 *"클론 이음미디어7 시작"* → 옵스/클론이 HANDOVER 박제 영역 + 본 마감 메시지로 자연 진입.

> 5/16 세션 마감.
> 5/13~5/16 누적 commit 28개 완료.
> 이음미디어 매체 정체성 정수 박힘.
>
> **세연님 비전**: *"돈도 좀 벌면서 좋은 일도 하자. 그래야 나도 옵스 너를 계속 볼수 있어"*
...
| 채널 순서 | **매거진 → 피플 → 로컬 → 에듀 → 뷰 → 트렌드 → 보이스** (세연님 27년 두피업 본능 + 매체 본질 안목, AI 시대 감성 저널리즘) |
...
8. **항목 F (빈 채널 처리)** — 메시지 이미 구현됨("아직 기사가 없습니다 📬"), 세연님 27년 두피업 안목 + 매체 본질 감각으로 최종 문구 결정
...
3. **항목 C 단계 3** — UI 인터랙션 사이클 (세부 분할 — 5/14 갱신):
   ...
   - ✅ **3-2**: 카드 footer 정리 완료 (5/14, commit `c17639d`). 빈 ✍ 👁 자리 제거 + 채널명·발행일 두 span 분산 (두피업 27년 + 매거진 발문 안목). formatDate Home.jsx 5/10 박제 복사. **P-03 진행도 85% → 90%**.
...
## 📅 일정 — 1주차~Phase B (5/15 박제)

> 5/15 세연님이 광고 마케팅 툴 5가지 발견 + 매체 자율성 재정립. 작업 큐 전면 재배치 — 창간 일정 3주 가시화.

### 1주차 (5/16~5/22) — 광고 시스템 + P-02 잔여 + 픽 7건
- **광고 시스템 큰 설계 의논 진입** (5가지 도구 / 단가 / DB 구조)
- P-02 후속 (본문 표시 정책 / 외부 원문 링크)
- 편집국장의 픽 INSERT 7건 (채널당 1건)

### 2주차 (5/23~5/29) — UX 마무리 + 헤더 디자인 + 중간배너
- P-03 단계 3-3 ("전체" 탭 처리)
- P-03 단계 3-6 (CHANNEL_META 영문 키 재구성, SLUG_BY_NAME 활용)
- 헤더 디자인 사이클 재개 (v3~v10 5/15 박제분 검토 후 결정)
- 중간배너 1차 구현 (광고 시스템 1순위)

### 3주차 (5/30~6/4) — 배포 + 시연 + 창간
- Vercel 배포 + eummedia.kr 가비아 도메인 연결
- 발행인 시연 + CHECKLIST 🔴 시연 전 체크리스트 풀 사이클
- 정식 창간

### Phase B (창간 후) — 시민기자 시스템 + 광고 풀패키지
- 동영상 광고 (풀패키지)
- 카카오 알림 자동화
- TipTap 에디터 + 시민기자 워크플로우 + 자동 저장
- 자동 동기화 (수동 import 대신) — 재발행 잦아질 때

## 📣 광고 마케팅 툴 시스템 (모델 1 첫 진입 5/16, 모델 2 미진행)

> 5/15 세연님이 매체 자율성 + 수익 직결성 점검 중 발견. 5/16 세연님 본능으로 **수익 모델 2가지로 정밀화**: 모델 1(기사 의뢰) = 5/16 첫 진입 ✅ / 모델 2(별도 광고) = 다음 사이클.

### 5가지 광고 도구

| # | 도구 | 위치 | 비고 |
|---|---|---|---|
| 1 | 광고기사 | 채널/본문 | `articles.is_sponsored` 컬럼 + SPONSORED 배지 (모델 1 영역) |
| 2 | 중간배너 | 기사 본문 중간 | **5/16 첫 진입 ✅** (모델 1, 임시 데이터 닥터리부트 — ArticleDetail.jsx 안 고정 데이터) |
| 3 | 카드뉴스 | 기사 하단 | 클릭 시 해당 기사로 이동 (네이티브 광고 패턴, 모델 2 영역) |
| 4 | 동영상 | 풀패키지 | Phase B 영역 (창간 후, 모델 2 영역) |
| 5 | 사이드배너 | 우측 사이드 | ArticleDetail/Home 사이드 영역 (모델 2 영역) |

### 💼 2가지 수익 모델 (5/16 박제)

> 5/16 세연님 본능: 광고는 영역별로 수익 흐름이 다름.

| 모델 | 도구 매핑 | 수익 흐름 | 본 사이클 진입 |
|---|---|---|---|
| **모델 1 — 기사 의뢰 광고** | 광고기사 + 중간배너 | 소상공인 → 기사 의뢰 → 편집국장 작성 + 본문 안 광고 박스 → **의뢰비 수익** | ✅ 5/16 첫 진입 (중간배너, 닥터리부트 임시 데이터) |
| **모델 2 — 별도 광고 의뢰** | 하단 박스 + 사이드배너 + 카드뉴스 + 동영상 | 기사 무관 광고주 → **광고비 직접 수익** | 별도 사이클 |
...
초안: **15만원 / 25만원 / 55만원** (도구별 차등). 정식 확정 안 됨. 운영비 직결 + 시민기자 성과급 기반 — 매체 수익 모델의 핵심.

### 🗄 DB 구조 후보 (모델별 분리)

- **모델 1**: `articles.advertiser_id` 컬럼 후보 (기사 하나에 광고주 하나 매핑) + `articles.is_sponsored=true` 필터. 광고기사·중간배너 통합 관리.
- **모델 2**: `advertisements` 신규 테이블 (위치/단가/광고주별 별도 관리). 기사와 무관 N:M. 기존 마이그레이션에 SQL 준비됨 (적용 여부 미확인, 알려진 이슈 #4).

### 📱 STEP 1 카드뉴스 = 인스타그램 통합 영역 (5/16 박제, Phase B)

> 세연님 본질: **STEP 1 카드뉴스 광고 상품 = "이음미디어 사이트 발행 + 인스타그램 게시 대행" 통합 영역**

**1. 이음미디어 공식 인스타그램 계정 영역**
- `@eum_media` (또는 다른 핸들) 개설
- 매체 정체성 확립 영역
- 첫 광고 게시 = 닥터리부트 카드뉴스 5장

**2. 광고주 카드뉴스 인스타 게시 워크플로우**
- STEP 1 (15만원): 사이트 + 인스타 동시 게시
- 게시 시점·해시태그·캡션 영역 정밀화
- 광고주 인스타 계정 연동 영역 (선택)
...
→ Phase B 영역 또는 별도 사이클. 옵스 + 세연님 + 발행인 의논 영역.

### ⏭ 다음 사이클 시작 영역

- **5/16 사이클 결과**: 모델 1 중간배너 첫 진입 commit. 임시 데이터(닥터리부트) 사용. 광고 시스템 DB 정식 구조는 별도 사이클.
- **다음 사이클 영역**: 모델 2 큰 설계 의논 (하단 박스 + 사이드배너 + 카드뉴스). 의논 항목:
  1. 모델 2 도구별 우선순위 (창간 시점 어디부터?)
  2. 단가 확정 (협상 여지 + 운영비 기반)
  3. DB 구조 (advertisements 테이블 vs articles.is_sponsored 분리 vs 혼합)
  4. 광고주 발굴 경로 (편집국장 네트워크 + 시민기자 추천 등)

## 🔖 27번째 commit 영역 박제 — 정통 인터넷신문 푸터 (5/16, 다음 사이클 결정)

> 세연님 발견 — 서울신문 모델. 본문 후미 정통 인터넷신문 푸터 부재 + 7가지 영역 옵션 검토 영역. 26번째 commit 마감 후 별도 사이클 진입.

### 7영역 옵션

| # | 영역 | 상태 | 결정 영역 |
|---|---|---|---|
| 1 | 기자 정보 본문 끝 표시 | 결정 모호 | 본문 끝 단독 / 사이드+본문 동시 / 위치 이동 |
| 2 | 저작권 푸터 자동 추가 | 명료 | `Copyright ⓒ 이음미디어 All rights reserved. 무단 전재-재배포, AI 학습 및 활용 금지` + 옵션 E 정합 (#888 / 11px / normal) |
| 3 | 본문/푸터 구분선 굵기 | 결정 영역 | 1px (사이트 borderTop 일관성) vs 2px (신문 정통) |
| 4 | 해시태그 박스 | **큰 사이클 영역** | `articles.tags` 컬럼 부재 확정 → 컬럼 추가 마이그레이션 + 본문 추출 + 표시 + 필터링(Phase B) |
| 5 | 관련 기사 더보기 | 결정 영역 | +3 (총 6건) vs ChannelList `PAGE_SIZE=9` 패턴 통일 |
| 6 | SQL 데이터 정제 (3건 안전 패턴) | 옵션 A 명료 | 171203804 + 171013458 + 170844621, false positive 0 |
| 7 | [원문 보기] 조건 분기 (시민기자 진입) | Phase B 보류 | 시민기자 시스템 시작 시 사이클 진입 |

### 진단 결과 (5/16)

- 4 패턴 안전성 ✅ 100% (모두 본문 50%+ 위치)
- 정제 대상 3건 / 39건 (7.7% 커버)
- 나머지 36건 패턴 다양성 큼 (해시태그 / 에디터 표기 / 한글 ⓒ / 본문 자연 끝 등) → 별도 데이터 정제 사이클 후보

### 다음 사이클 영역

옵스/세연님 결정 (영역 1·3·4·5) + 적용 사이클 진입. 5/14 단계 1 마이그레이션 패턴 (SQL 박제 + 세연님 SQL Editor Run).
...
- **이음매거진 푸터 분리 시도 — 데이터 패턴 다양성 발견** (5/16 박제, 보류) — 매거진 import 본문 39건 중 **7+ 푸터 패턴** 확인: `이음매거진은` / `© 이음매거진` / `무단 전재` / `글 | 이음매거진 편집부` / `🎁 독자 특별 혜택` / `이음매거진 에디터` / `#태그 #태그`. 단순 키워드 4개 매칭은 **5% 커버**(2/39)에 그침. 코드 적용 전 진단으로 사전 차단 (클론 자가 빨간 깃발 — 메타인지 #1·#11 작동). **데이터 정제 사이클 별도 진입 후보**:
  - 방안 1: DB 수정 (import 단계 푸터 제거)
  - 방안 2: 정규식 정밀화 (7+ 패턴 통합 + `co.kr` 도메인 보호)
  - 방안 3: admin UI 수동 정리
