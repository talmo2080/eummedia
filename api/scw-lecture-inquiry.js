/* global process */
// =============================================================
// 성창운 총장 강의 문의 — 텔레그램 알림 + lecture_inquiries 저장
// POST /api/scw-lecture-inquiry
// =============================================================
...
export default async function handler(req, res) {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    ...
    console.warn('[scw-lecture-inquiry]', insertError);
  } else {
    try {
      ...
      if (error) {
        ...
        console.error('[scw-lecture-inquiry] supabase error:', insertError);
      } else {
        insertOk = true;
      }
    } catch (err) {
      insertError = err?.message || String(err);
      console.error('[scw-lecture-inquiry] exception:', insertError);
    }
  }
  ...
  if (tgToken && tgChatId) {
    ...
    try {
      ...
      if (tgRes.ok) {
        ...
        console.error('[scw-lecture-inquiry] telegram error:', telegramError);
      }
    } catch (err) {
      telegramError = err?.message || String(err);
      console.error('[scw-lecture-inquiry] telegram exception:', telegramError);
    }
  } else {
    telegramError = 'TELEGRAM env missing';
    console.warn('[scw-lecture-inquiry] telegram env missing');
  }
