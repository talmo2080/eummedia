/* global process */
// =============================================================
// 이광우 대표 힐링경영 문의 — 텔레그램 알림 + lecture_inquiries 저장
// POST /api/lwk-lecture-inquiry
// 알림 수신: 이광우 대표님 개인 chat_id (TELEGRAM_LWK_CHAT_ID)
...
export default async function handler(req, res) {
  ...
  // 이광우 대표님 개인 chat_id 우선, 없으면 세연님(안전장치)
  const tgChatId       = process.env.TELEGRAM_LWK_CHAT_ID || process.env.TELEGRAM_CHAT_ID;
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    ...
    console.warn('[lwk-lecture-inquiry]', insertError);
  } else {
    try {
      ...
      if (error) {
        ...
        console.error('[lwk-lecture-inquiry] supabase error:', insertError);
      } else {
        insertOk = true;
      }
    } catch (err) {
      insertError = err?.message || String(err);
      console.error('[lwk-lecture-inquiry] exception:', insertError);
    }
  }

  // (2) 텔레그램 알림 → 이광우 대표님 개인
  let telegramOk = false;
  let telegramError = null;
  if (tgToken && tgChatId) {
    const text =
      '🌿 <b>힐링숲라파 문의가 도착했습니다!</b>\n' +
      '────────────\n' +
      `👤 이름: ${escHtml(name)}\n` +
      `📞 연락처: ${escHtml(phone)}\n` +
      `📝 문의 내용:\n${escHtml(message)}\n` +
    ...
    try {
      ...
      if (tgRes.ok) {
        ...
        console.error('[lwk-lecture-inquiry] telegram error:', telegramError);
      }
    } catch (err) {
      telegramError = err?.message || String(err);
      console.error('[lwk-lecture-inquiry] telegram exception:', telegramError);
    }
  } else {
    telegramError = 'TELEGRAM env missing';
    console.warn('[lwk-lecture-inquiry] telegram env missing');
  }
